...
public class CategoryServiceImpl implements CategoryService {
	...
	public CategoryDto updateCategory(CategoryDto categoryDto, Integer categoryId) {
		Category cat = this.categoryRepo.findById(categoryId).orElseThrow(
				()->new ResourceNotFoundException("Catgory","Catgory Id", categoryId));
		
		cat.setCategoryTitle(categoryDto.getCategoryTitle());
	    cat.setCategoryDescription(categoryDto.getCategoryDescription());
		Category addct = this.categoryRepo.save(cat);
		return this.modelMapper.map(addct, CategoryDto.class);
	}
